    ...
    double visE=0;
       ...
       for(int i=0; i<nh; i++){
            ...
            if (apdg==2212 || apdg==2112){
                mult++;
                if (apdg==2212) leadP = std::max(leadP, KE);
                if (apdg==2212) diff++;
                else diff--;
            }
        ...
        double Ebias = 0.0;
        if (mode==kKO)
            Ebias = (KEini - leadP) / KEini;
        else 
            Ebias = (Eini - leadP) / Eini;
...
// =======================================================
void plotLeadingPwAllRW(int probe=211, double ke=0.675, int target=1000180400,
...
h4->Scale(1.0 / h4->Integral());
    ...
    h1->GetXaxis()->SetTitle("(E^{ini}-T^{leading}_{p})/E^{ini}");
if (probe>211) h1->GetXaxis()->SetTitle("(T^{ini}-T^{leading}_{p})/T^{ini}");
...
leg->Draw();
    ...
    // ---------------------------------------------------
    std::string channelStr="leadingPThreshPionAbs";

    if (mode == kAbs)
        channelStr = useThreshold ?
            "leadingPThreshPionAbs" :
            "leadingPPionAbs";
    if (mode==kKO)
        channelStr = useThreshold ?
            "leadingPThreshKO" :
