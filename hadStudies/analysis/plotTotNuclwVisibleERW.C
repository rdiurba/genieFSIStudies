...
TH1D* makeHistogram(TTree* t, std::string label, Mode mode, bool useThreshold,int A1, int target, bool compound, bool RW, std::string pstr="pionp",std::string reweightModel="INCL++")
{
    ...
    TH1D *hist = new TH1D(Form("hist_%s",label.c_str()),
                          Form("hist_%s",label.c_str()),40,0,40);

    if (!t || t->GetEntries() < 1) return hist;

    int pdgh[100], nh, probe_fsi;
    double Eh[100], mh[100];
    int np; int nn; double KEini; double Eini;
...
// =======================================================
void plotTotNuclwVisibleERW(int probe=211, double ke=0.175, int target=1000180400,
...
leg->Draw();
    ...
    // ---------------------------------------------------
    std::string channelStr="nucleonSumThreshPionAbsVisERW";

    if (mode == kAbs)
        channelStr = useThreshold ?
            "nucleonSumThreshPionAbsVisERW" :
            "nucleonSumPionAbsVisERW";
    if (mode==kKO)
        channelStr = useThreshold ?
            "nucleonSumThreshKOVisERW" :
            "nucleonSumKOVisERW";
