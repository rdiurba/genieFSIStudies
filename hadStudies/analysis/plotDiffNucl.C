...
// =======================================================
// Histogram builder (difference: np - nn)
// =======================================================
TH1D* makeHistogram(TTree* t, std::string label, Mode mode, bool useThreshold,int A1, bool compound)
{
    ...
    TH1D *hist = new TH1D(Form("hist_%s",label.c_str()),
                          Form("hist_%s",label.c_str()),max-min,min,max);
    ...
    for (Long64_t j=0; j<t->GetEntries(); j++){
       ...
       for(int i=0; i<nf; i++){
        ...
        if (useThreshold==0 && compound==false) diff=np-nn;
        hist->Fill(diff);
    }
...
// =======================================================
void plotDiffNucl(int probe, double ke, int target,
...
leg->Draw();
    ...
    // Output plot
    // ---------------------------------------------------
    std::string channelStr="nucleonDiffThreshPionAbs";

    if (mode == kAbs)
        channelStr = useThreshold ?
            "nucleonDiffThreshPionAbs" :
            "nucleonDiffPionAbs";
    if (mode==kKO)
        channelStr = useThreshold ?
            "nucleonDiffThreshKO" :
    ...
    c1->Print(Form("../plotting/%s_%s_%s_%d_%d_%d.pdf",
        ...
        // ---------------------------------------------------
    // Gaussian fits (ALL cases)
    // ---------------------------------------------------
    TF1* g1 = new TF1("g1","gaus",min,max);
    TF1* g2 = new TF1("g2","gaus",min,max);
    TF1* g3 = new TF1("g3","gaus",min,max);
    TF1* g4 = new TF1("g4","gaus",min,max);
    ...
    if (mode==kAbs){
        if (useThreshold && !compound)
            file.open("pionAbsDifferenceThresh.txt", std::ios_base::app);
        else if(compound)             file.open("pionAbsDifferenceCompoundThresh.txt", std::ios_base::app);
        else
            file.open("pionAbsDifference.txt", std::ios_base::app);
    } else {
                if (useThreshold && !compound)
            file.open("nucleonKODifferenceThresh.txt", std::ios_base::app);
        else if(compound)             file.open("nucleonKODifferenceCompoundThresh.txt", std::ios_base::app);
       else file.open("nucleonKODifference.txt", std::ios_base::app);
    }

    file << probe << "," << ke << "," << target << ","
         << g1->GetParameter(1) << "," << g1->GetParError(1) << ","
         << g1->GetParameter(2) << "," << g1->GetParError(2) << ","
         << g2->GetParameter(1) << "," << g2->GetParError(1) << ","
         << g2->GetParameter(2) << "," << g2->GetParError(2) << ","
         << g3->GetParameter(1) << "," << g3->GetParError(1) << ","
         << g3->GetParameter(2) << "," << g3->GetParError(2) << ","
         << g4->GetParameter(1) << "," << g4->GetParError(1) << ","
         << g4->GetParameter(2) << "," << g4->GetParError(2)
         << std::endl;
