...
TH1D* makeHistogram(TTree* t, std::string label, Mode mode, bool useThreshold,int A1, bool compound)
{
    ...
    TH1D *hist = new TH1D(Form("hist_%s",label.c_str()),
                          Form("hist_%s",label.c_str()),50,0,1);
...
// =======================================================
void plotELeading(int probe, double ke, int target,
                  bool useThreshold = false, bool compound=false)
{
  ...
  TCanvas *c1 = new TCanvas("c1","c1",200,10,700,500);
    ...
    if (useThreshold)
        h1->GetXaxis()->SetTitle("Kinetic Energy of the Leading Proton [GeV]");
    else
        h1->GetXaxis()->SetTitle("Kinetic Energy of the Leading Proton [GeV]");
...
leg->Draw();
    ...
    // ---------------------------------------------------
    std::string channelStr="highKEProtonSumThreshPionAbs";

    if (mode == kAbs)
        channelStr = useThreshold ?
            "highKEProtonThreshPionAbs" :
            "highKEProtonSumPionAbs";
    if (mode==kKO)
        channelStr = useThreshold ?
            "highKEProtonSumThreshKO" :
            "highKEProtonSumKO";
