...
for particle in probe:
    ...
    for t in tgt:
        for k in ke:
            seed=seed+1
            ghepFile=path+""+str(probeStr)+"_"+str(t)+"_"+str(k)+"GeV_HINCL_1M"
            gstFile=path+""+str(probeStr)+"_"+str(t)+"_"+str(k)+"GeV_HINCL_1M.ginuke.root"
            os.system("gevgen_hadron --seed "+str(seed)+" -n "+str(num)+" -p "+str(particle)+" -t "+str(t)+" -k "+str(k)+" -m HINCL -o "+str(ghepFile))
