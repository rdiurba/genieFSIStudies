...
std::string deuteronCut="np==2 && nn==0";
if (probe==111) deuteronCut="np==1 && nn==1";
if (probe==-211) deuteronCut="np==0 && nn==2";
  deuteronCut="np<0"; 
  //if (label=="20i") t->Project(Form("hist_Mult%s",label.c_str()),"np-nn",Form("probe_fsi==5 && !(%s)",deuteronCut.c_str()));
   ...
   if (abs(pdgf[i])==2112 && Ef[i]-mp>0.02) mult=mult-1;
...
void plotAbsDiffThresh(int probe=211, double ke=0.125, int target=1000822080)
{
...
TH1D* mult20l=makeHistogram(t_nu20l,"20l",probe);
  ...
  lErr->AddEntry(mult20l,"GENIE v3.4 Geant4","l");
   ...
   lErr->Draw("SAME");
   c1->Print(Form("nucleonThreshDiffPionAbs_%s_%s_%d.png",probeStr.c_str(),keStr.c_str(),target));
      c1->Print(Form("nucleonThreshDiffPionAbs_%s_%s_%d.pdf",probeStr.c_str(),keStr.c_str(),target));
