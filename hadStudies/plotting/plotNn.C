...
//deuteronCut="np<0";
   ...
   if (label=="20i") t->Project(Form("hist_Mult%s",label.c_str()),"nn",Form("probe_fsi==5 && !(%s)",deuteronCut.c_str()));
   else t->Project(Form("hist_Mult%s",label.c_str()),"nn",Form("probe_fsi==5 && !(%s)",deuteronCut.c_str()));
   return hist_Mult;
}



void plotNn(int probe=211, double ke=0.125, int target=1000822080)
{
...
TH1D* mult20l=makeHistogram(t_nu20l,"20l",probe);
   ...
   mult20i->SetTitle(Form("Number of Neutrons in Pion Absorption"));
   mult20i->GetXaxis()->SetTitle("N_{n}");
  ...
  lErr->AddEntry(mult20l,"GENIE v3.4 Geant4","l");
   ...
   lErr->Draw("SAME");
   c1->Print(Form("neutronMultPionAbs_%s_%s_%d.png",probeStr.c_str(),keStr.c_str(),target));
...
std::ofstream file;
file.open("pionNnSum.txt",std::ios_base::app);
