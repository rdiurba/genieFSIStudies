...
TH1D *hist_Mult=new TH1D(Form("hist_Mult%s",label.c_str()),Form("hist_Mult%s",label.c_str()),15,0,15);
   ...
   for(int i=0; i<nf; i++){
   if ((abs(pdgf[i])==211 && Ef[i]-mpi>threshPi.at(thresh)) || pdgf[i]==111) mult++;
...
void plotPionDials(int thresh=0)
{
...
TH1D* profile20i=makeProfile(t_numu,t_weights,"20i",400000,thresh);
   ...
   mult20i->SetTitle(Form("Multiplicity of Pions with Min. Length %s",label.c_str()));
   mult20i->GetXaxis()->SetTitle("Number of Pions");
  ...
  lOnlyExtreme->AddEntry(mult20l,"Geant4","l");
   ...
   lErr->Draw("SAME");
   c1->Print(Form("pionMultNominal_%d.png",thresh));
   mult20i->Draw("E0 P");
   mult20k->Draw("HIST SAME");
   mult20l->Draw("HIST SAME");
   lOnlyExtreme->Draw("SAME");
   c1->Print(Form("pionMultNominalWeights_%d.png",thresh));
}
