...
#include <vector>
#include <map>
TH1D* makeHistogram(TTree* t, std::string label,  int numEvents=0, int thresh=0, int probe=211){
TH1D *hist_Mult=new TH1D(Form("hist_Mult%s",label.c_str()),Form("hist_Mult%s",label.c_str()),15,0,15);
   ...
   int fspl; double El; int nipi0; int nipip; int nipim;
   ...
   if(nipip!=1 || nipim!=0 || nipi0!=0) continue;
   for(int i=0; i<nf; i++){
  if (probe!=111) {if (pdgf[i]==probe && Ef[i]-mpi>threshPi.at(thresh)) mult++;}
   else if (probe!=2212) {if (pdgf[i]==probe && Ef[i]-mpi>threshP.at(thresh)) mult++;}
   else if (probe!=2112) {if (pdgf[i]==probe) mult++;}
     else {if (pdgf[i]==probe) mult++;}
...
TProfile* makeProfile(TTree* t, TTree* t_weights,  std::string label,  int numEvents=0, int thresh=0, int probe=211){
...
TH1D *hist_Mult20i=new TH1D(Form("hist_Mult20i%s",label.c_str()),Form("hist_Mult20i%s",label.c_str()),15,0,15);
   ...
   double mk=0.4937; double mp=0.9383; double mpi=0.1396; double mmu=0.1057;
   int pdgf[100]; int nf; double Ef[100];
   int fspl; double El; int nipip; int nipim; int nipi0;
   if (numEvents==0) numEvents=t->GetEntries();
   t->SetBranchAddress("pdgf",&pdgf);
   t->SetBranchAddress("nf",&nf);
   t->SetBranchAddress("Ef",&Ef);
   ...
   //if (abs(fspl)==13 && El-mmu>threshMu.at(thresh)) mult++;
    ...
    //hist_Mult20i_multisimVec.at(i)->Fill(mult,weights20iPi[i]);
    if (probe==2212 || probe==2112) hist_Mult20i_multisimVec.at(i)->Fill(mult,weights20iPi[i]*weights20iP[i]);
    else hist_Mult20i_multisimVec.at(i)->Fill(mult,weights20iPi[i]);
...
void plotSinglePionDials(int thresh=0, int probe=211)
{
...
gStyle->SetOptFit(111);
  ...
  TCanvas *c1 = new TCanvas("c1","c1",200,10,700,500);
   ...
   TH1D* mult20i = makeHistogram(t_numu,"20i",400000,thresh,probe);
   TH1D* mult20j = makeHistogram(t_nu20j,"20j",2000000,thresh,probe);
   TH1D* mult20k = makeHistogram(t_nu20k,"20k",200000,thresh,probe);
   TH1D* mult20l = makeHistogram(t_nu20l,"20l",500000,thresh,probe);
   TH1D* mult21 = makeHistogram(t_nu21,"G21",200000,thresh,probe);
TH1D* profile20i=makeProfile(t_numu,t_weights,"20i",400000,thresh,probe);
   ...
   mult20i->SetTitle(Form("Final State of 1#pi^{+} Evts. with Min. Len. %s",label.c_str()));
   mult20i->GetXaxis()->SetTitle("Number of Final State #pi^{+}");
   if (probe==111)     mult20i->GetXaxis()->SetTitle("Number of Final State #pi^{0}");
   if (probe==-211)     mult20i->GetXaxis()->SetTitle("Number of Final State #pi^{-}");
   if (probe==2212)     mult20i->GetXaxis()->SetTitle("Number of Final State p^{+}");
   if (probe==2112)     mult20i->GetXaxis()->SetTitle("Number of Final State Neutrons");
   mult20i->GetYaxis()->SetTitle("Fraction of Events");
   gPad->SetLeftMargin(0.15);
   mult20i->GetYaxis()->SetRangeUser(0,1.0);
  ...
  lOnlyExtreme->AddEntry(mult20l,"Geant4","l");
   ...
   lErr->Draw("SAME");
   c1->Print(Form("singlePionMultNominal_%d_%d.png",thresh,probe));
   mult20i->Draw("E0 P");
   mult20k->Draw("HIST SAME");
   mult20l->Draw("HIST SAME");
   lOnlyExtreme->Draw("SAME");
   c1->Print(Form("singlePionNominalWeights_%d_%d.png",thresh, probe));
}
