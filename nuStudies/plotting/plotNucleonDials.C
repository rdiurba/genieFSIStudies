...
TProfile* makeProfile(TTree* t, TTree* t_weights,  std::string label,  int numEvents=0, int thresh=0, int allRW=0){
...
TH1D *hist_Mult20i=new TH1D(Form("hist_Mult20i%s",label.c_str()),Form("hist_Mult20i%s",label.c_str()),15,0,15);
   ...
   if (allRW==0) hist_Mult20i_multisimVec.at(i)->Fill(mult,weights20iP[i]);
   else hist_Mult20i_multisimVec.at(i)->Fill(mult,weights20iP[i]*weights20iPi[i]);
	}
   }
     for (int i=0; i<100; i++){
...
void plotNucleonDials(int thresh=0, int rwAll=0)
{
...
TH1D* profile20i=makeProfile(t_numu,t_weights,"20i",400000,thresh,rwAll);
   ...
   mult20i->SetTitle(Form("Multiplicity of Protons with Min. Length %s",label.c_str()));
   mult20i->GetXaxis()->SetTitle("Number of Protons");
   mult20i->GetYaxis()->SetTitle("Fraction of Events");
   gPad->SetLeftMargin(0.15);
   mult20i->GetYaxis()->SetRangeUser(0,0.6);
  ...
  if (rwAll==0) lOnlyExtreme->AddEntry(mult20i,"hA with Nucl. FSI Uncert.","l");
  else lOnlyExtreme->AddEntry(mult20i,"hA with All FSI Uncert.","l");
  ...
  lOnlyExtreme->AddEntry(mult20l,"Geant4","l");
   ...
   lErr->Draw("SAME");
   c1->Print(Form("protonMultNominal_%d_%d.png",thresh,rwAll));
   mult20i->Draw("E0 P");
   mult20k->Draw("HIST SAME");
   mult20l->Draw("HIST SAME");
   lOnlyExtreme->Draw("SAME");
   c1->Print(Form("protonMultNominalWeights_%d_%d.png",thresh,rwAll));
}
