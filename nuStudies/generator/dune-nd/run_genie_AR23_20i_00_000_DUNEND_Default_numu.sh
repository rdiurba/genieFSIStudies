...
gevgen -n $events -p ${probe} -t ${target} -e ${minE},${maxE} --message_thresholds $GENIE/config/Messenger_whisper.xml  --event-generator-list ${interaction} --tune ${tune} --cross-sections ${GENIEXSECFILE2} -f ${fluxfile},${fluxhisto} -o /dune/data/users/rdiurba/genieFSIStudies/DUNEND_${probe}_${target}_${interaction}_${tune}.ghep.root --seed 111111111
