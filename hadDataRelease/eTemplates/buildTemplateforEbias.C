...
/*
void normalizePerEnergy(TH3D* h)
{
    int nX = h->GetNbinsX();
    int nY = h->GetNbinsY();
    int nZ = h->GetNbinsZ();
    // Normalize each (ix, iy) column along Z
    for (int ix = 0; ix <= nX + 1; ++ix) {
        for (int iy = 0; iy <= nY + 1; ++iy) {
            double sum = 0.0;
...
*/
TH2* getHist(std::string simulation, int pdg, std::string particle, int target) { // get histogram from the TTree (e.g. gst/ginuke file)
  ...
  const int nummax = 99;
    ...
    TH2D* h_KEini_Ebias = new TH2D(Form("%s_%s_KEini_vs_Ebias", simulation.c_str(), particle.c_str()), Form("%s_%s_%d_KEini_vs_Ebias", simulation.c_str(), particle.c_str(),target), 40, 0, 2,  20, -1, 1);

    TH3D* hist;
    for (int i=1; i<40; i++){
    TFile* file = new TFile(Form("$PSCRATCH/hadronFiles/%s_%d_%gGeV_%s_100k.ginuke.root",particle.c_str(),target,v.at(i),simulation.c_str() ));
    ...
    for (int i=0; i<tree->GetEntries(); ++i) {
      ...
      if ( (fsi!=1) && KEini>0&&KEini<20) { // selections (e.g. exclusive channel & KE range)
...
void buildTemplate(int pdg=211, int target=1000180400) {
  ...
  string particle;
    ...
    hA2018_hist = (TH3*)getHist(Form("hA2018"), pdg, particle,target);
    hN2018_hist = (TH3*)getHist(Form("hN2018"), pdg, particle,target);
    INCL_hist = (TH3*)getHist(Form("HINCL"), pdg, particle,target);
    G4BC_hist = (TH3*)getHist(Form("HG4BertCasc"), pdg, particle,target);
  ...
  hA2018_hist->SetName(Form("hA2018_%s_%d_KEini_vs_Ebias", particle.c_str(),target));

  hN2018_hist->SetName(Form("hN2018_%s_%d_KEini_vs_Ebias", particle.c_str(),target));
  INCL_hist->SetName(Form("INCL++_%s_%d_KEini_vs_Ebias", particle.c_str(),target));
  G4BC_hist->SetName(Form("Geant4_%s_%d_KEini_vs_Ebias", particle.c_str(),target));

  hN2018_hist->SetTitle(Form("hN2018_%s_%d_KEini_vs_Ebias", particle.c_str(),target));
  INCL_hist->SetTitle(Form("INCL++_%s_%d_KEini_vs_Ebias", particle.c_str(),target));
  G4BC_hist->SetTitle(Form("Geant4_%s_%d_KEini_vs_Ebias", particle.c_str(),target));
  hA2018_hist->SetTitle(Form("hA2018_%s_%d_KEini_vs_Ebias", particle.c_str(),target));
  TFile *outfile = new TFile("FSI_Evis_reweight_template.root","UPDATE");
  hA2018_hist->Write(Form("hA2018_%s_%d_KEini_vs_Ebias", particle.c_str(),target));
  hN2018_hist->Write(Form("hN2018_%s_%d_KEini_vs_Ebias", particle.c_str(),target));
  INCL_hist->Write(Form("INCL++_%s_%d_KEini_vs_Ebias", particle.c_str(),target));
  G4BC_hist->Write(Form("Geant4_%s_%d_KEini_vs_Ebias", particle.c_str(),target));

  outfile->Write();  outfile->Close(); }
void buildTemplateforEbias(){
